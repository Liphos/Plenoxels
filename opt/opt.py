...
from util.util import Timing, generate_dirs_equirect, get_expon_lr_func, viridis_cmap
...
group.add_argument(
    "--init_iters", type=int, default=0, help="do not upsample for first x iters"
)
group.add_argument(
...
group.add_argument(
    "--sh_dim", type=int, default=9, help="SH/learned basis dimensions (at most 10)"
)
...
group.add_argument(
    "--lr_sigma_bg", type=float, default=3e0, help="SGD/rmsprop lr for background"
)
group.add_argument(
    "--lr_sigma_bg_final",
    type=float,
...
group.add_argument(
    "--lr_color_bg", type=float, default=1e-1, help="SGD/rmsprop lr for background"
)
group.add_argument(
    "--lr_color_bg_final",
    type=float,
...
group.add_argument(
    "--lr_basis", type=float, default=1e-6, help="SGD/rmsprop lr for SH"  # 2e6,
)
...
group.add_argument(
    "--rms_beta", type=float, default=0.95, help="RMSProp exponential averaging factor"
)

group.add_argument("--print_every", type=int, default=20, help="print every")
group.add_argument("--save_every", type=int, default=5, help="save every x epochs")
...
group.add_argument(
    "--init_sigma_bg", type=float, default=0.1, help="initialization sigma (for BG)"
)
...
if args.save_img_disk:
    os.makedirs(args.train_dir + "/images/", exist_ok=True)
...
if args.background_nlayers > 0 and not dset.should_use_background:
    warn(
        "Using a background model for dataset type "
        + str(type(dset))
        + " which typically does not use background"
    )
...
resample_cameras = [
    ...
    for i, c2w in enumerate(dset.c2w)
]
...
if args.enable_random:
    warn(
        "Randomness is enabled for training (normal for LLFF & scenes with background)"
    )
...
while True:
    ...
    def eval_step():
        ...
        with torch.no_grad():
                ...
                if i % img_save_interval == 0:
                    ...
                    if args.save_img_disk:
                        ...
                        image = Image.fromarray(
                            (img_pred.numpy() * 255).astype(np.uint8)
                        )

                        # Save the image to disk
                        image.save(
                            args.train_dir
                            + f"/images/image_{epoch_id}_{img_id:04d}.png",
                            format="PNG",
                        )
                    ...
                    if args.log_mse_image:
                        ...
                        summary_writer.add_image(
                            f"test/mse_map_{img_id:04d}",
                            mse_img,
                            global_step=gstep_id_base,
                            dataformats="HWC",
                        )
                    if args.log_depth_map:
                        depth_img = grid.volume_render_depth_image(
                            cam,
                            args.log_depth_map_use_thresh
                            if args.log_depth_map_use_thresh
                            else None,
                        )
                        depth_img = viridis_cmap(depth_img.cpu())
                        summary_writer.add_image(
                            f"test/depth_map_{img_id:04d}",
                            depth_img,
                            global_step=gstep_id_base,
                            dataformats="HWC",
                        )
            ...
            if (
                ...
                or grid.basis_type == svox2.BASIS_TYPE_MLP
            ):
                ...
                sphfuncs = (
                    sphfuncs.view(EQ_RESO, EQ_RESO * 2, -1)
                    .permute([2, 0, 1])
                    .cpu()
                    .numpy()
                )
                ...
                stats = [
                    (sphfunc.min(), sphfunc.mean(), sphfunc.max())
            ...
            for stat_name in stats_test:
                summary_writer.add_scalar(
                    "test/" + stat_name,
                    stats_test[stat_name],
                    global_step=gstep_id_base,
                )
            summary_writer.add_scalar(
                "epoch_id", float(epoch_id), global_step=gstep_id_base
            )
            print("eval stats:", stats_test)

    if (
        epoch_id % max(factor, args.eval_every) == 0
    ):  # and (epoch_id > 0 or not args.tune_mode):
    ...
    def train_step():
        print("Train step")
        pbar = tqdm(
            enumerate(range(0, epoch_size, args.batch_size)), total=batches_per_epoch
        )
        ...
        for iter_id, batch_begin in pbar:
            ...
            lr_basis = (
                lr_basis_func(gstep_id - args.lr_basis_begin_step) * lr_basis_factor
            )
            lr_sigma_bg = (
                lr_sigma_bg_func(gstep_id - args.lr_basis_begin_step) * lr_basis_factor
            )
            lr_color_bg = (
                lr_color_bg_func(gstep_id - args.lr_basis_begin_step) * lr_basis_factor
            )
            ...
            if (iter_id + 1) % args.print_every == 0:
                ...
                if grid.basis_type == svox2.BASIS_TYPE_3D_TEXTURE:
                    summary_writer.add_scalar(
                        "lr_basis", lr_basis, global_step=gstep_id
                    )
                if grid.use_background:
                    summary_writer.add_scalar(
                        "lr_sigma_bg", lr_sigma_bg, global_step=gstep_id
                    )
                    summary_writer.add_scalar(
                        "lr_color_bg", lr_color_bg, global_step=gstep_id
                    )
    ...
    if (
        args.save_every > 0
        and (epoch_id + 1) % max(factor, args.save_every) == 0
        and not args.tune_mode
    ):
    ...
    if (gstep_id_base - last_upsamp_step) >= args.upsamp_every:
        ...
        if reso_id < len(reso_list) - 1:
            ...
            z_reso = (
                reso_list[reso_id]
                if isinstance(reso_list[reso_id], int)
                else reso_list[reso_id][2]
            )
            grid.resample(
                reso=reso_list[reso_id],
        ...
        if factor > 1 and reso_id < len(reso_list) - 1:
            print(
                "* Using higher resolution images due to large grid; new factor", factor
            )
            factor //= 2
